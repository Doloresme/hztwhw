(function () {
	...
	function constructUrl(url, o) {
		...
		for (var k in o) if (!isUndef(o[k]))
			if (Array.isArray(o[k]))
				o[k].forEach(function (item) {addPair(k, item)})
			else addPair(k, o[k]);
	...
	SocialShare.Weixin = function (data){
		openQrPopover('wxPop', share_on_moments,
			share_wechat_tip, data.qrcode);
	};
	...
	function openQrPopover(id, title, footnote, qrImageUrl) {
		...
		var element = $('<div class="QR_popover"><div class="head_">' +
			'<a class="close_" >×</a><h3>' + title + '</h3></div>' +
			'<div class="content_"><img src="../img/logo.png" data-init="0" class="qr-logo"></div>' +
			'<div class="footer_"><div>' + footnote + '</div></div></div>');
